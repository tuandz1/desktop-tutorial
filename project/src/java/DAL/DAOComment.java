...
public class DAOComment {
    ...
    public static void main(String[] args) {
        ...
        dao.searchReviewPro("S");
       List<Review> rv= dao.getCmt();
        for (Review review : rv) {
            System.out.println(review.getCommentText());
        }
    }
    ...
    public void getAllReviewPro() {
        cmt = new Vector<>();
        String sql = "SELECT c.CommentId,c.ProId,c.CommentText,c.Rating, c.CreatedBy,c.CreatedOn,a.avatar,a.full_name\n"
                + "FROM ProductComment c\n"
                + "JOIN Account a ON c.CreatedBy = a.id\n"
                + "order by c.ProId desc";
    ...
    public void searchReviewPro(String txt) {
        cmt = new Vector<>();
        String sql = "SELECT c.CommentId,c.ProId,c.CommentText,c.Rating, c.CreatedBy,c.CreatedOn,a.avatar,a.full_name\n"
                + "FROM ProductComment c\n"
                + "JOIN Account a ON c.CreatedBy = a.id\n"
                + "where c.CommentText like ? or a.full_name like ? \n"
                + "order by c.ProId desc";
    ...
    public void getAllProductPagging(List<Review> newsList, int start, int end) {
        cmt = new Vector<Review>();
        for (int i = start; i < end; i++) {
            cmt.add(newsList.get(i));
        }
    }

    public void updateContext(String content, int id) {
        String sql = "UPDATE [dbo].[ProductComment]\n"
                + "   SET [CommentText] = ?\n"
                + " WHERE CommentId = ?";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
